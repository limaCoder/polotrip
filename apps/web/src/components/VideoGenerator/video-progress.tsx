...
export function VideoProgress({ status, startedAt }: VideoProgressProps) {
  ...
  return (
    <div className={cn('flex flex-col items-center gap-4 rounded-xl p-6', config.bgClass)}>
      <Icon className={cn('h-12 w-12', config.colorClass, config.animate && 'animate-spin')} />
      <div className="text-center">
        <h4 className="font-semibold text-lg">{t(`status.${status}`)}</h4>
        <p className="text-muted-foreground text-sm">{t(`description.${status}`)}</p>
