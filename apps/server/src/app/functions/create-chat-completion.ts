...
import { closeMCPClient, getMCPToolsWithUserId } from "@/app/lib/mcp-client";
...
// Return type from streamText uses complex internal types that cannot be explicitly named.
// This is a known TypeScript limitation with the AI SDK's streamText return type.
// Using a wrapper function with explicit return type to avoid the type naming issue.
...
> {
  ...
  try {
    const result = streamText({
      ...
      messages,
      tools,
      // Allow up to 5 steps to enable multi-step tool calling
      // This allows the model to call tools and then generate text responses
      stopWhen: stepCountIs(5),
      onFinish: async () => {
        // Client is kept alive for reuse across requests
        // It will be closed automatically on process shutdown or on connection errors
      },
      onError: async (error) => {
        // Only close client on connection errors to allow retry
        if (
...
// Export with explicit type to avoid TypeScript's type naming limitation
