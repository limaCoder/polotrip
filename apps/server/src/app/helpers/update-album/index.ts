import type { db } from "@polotrip/db";
 ...
 */
async function updateSinglePhoto(
  tx: DbTransaction,
  ...
  if (photoUpdate.locationName !== undefined)
    updateData.locationName = photoUpdate.locationName;
  if (photoUpdate.latitude !== undefined)
    updateData.latitude = photoUpdate.latitude;
  if (photoUpdate.longitude !== undefined)
    updateData.longitude = photoUpdate.longitude;
  if (photoUpdate.description !== undefined)
    updateData.description = photoUpdate.description;
  if (photoUpdate.dateTaken !== undefined)
    updateData.dateTaken = photoUpdate.dateTaken;
 ...
 */
async function updateMultiplePhotos(
  tx: DbTransaction,
  ...
  if (fieldsToUpdate.locationName) {
    batchUpdateSQL = sql`${batchUpdateSQL}, "location_name" =
      CASE "id"
        ${sql.join(
          photoUpdates.map(
            (update) => sql`WHEN ${update.id} THEN ${update.locationName}`
          ),
          sql` `
        )}
        ELSE "location_name"
  ...
  if (fieldsToUpdate.latitude) {
    batchUpdateSQL = sql`${batchUpdateSQL}, "latitude" =
      CASE "id"
        ${sql.join(
          photoUpdates.map(
            (update) => sql`WHEN ${update.id} THEN ${update.latitude}`
          ),
          sql` `
        )}
        ELSE "latitude"
  ...
  if (fieldsToUpdate.longitude) {
    batchUpdateSQL = sql`${batchUpdateSQL}, "longitude" =
      CASE "id"
        ${sql.join(
          photoUpdates.map(
            (update) => sql`WHEN ${update.id} THEN ${update.longitude}`
          ),
          sql` `
        )}
        ELSE "longitude"
  ...
  if (fieldsToUpdate.description) {
    batchUpdateSQL = sql`${batchUpdateSQL}, "description" =
      CASE "id"
        ${sql.join(
          photoUpdates.map(
            (update) => sql`WHEN ${update.id} THEN ${update.description}`
          ),
          sql` `
        )}
        ELSE "description"
  ...
  if (fieldsToUpdate.dateTaken) {
    batchUpdateSQL = sql`${batchUpdateSQL}, "date_taken" =
      CASE "id"
        ${sql.join(
          photoUpdates.map(
            (update) => sql`WHEN ${update.id} THEN ${update.dateTaken}`
          ),
          sql` `
        )}
        ELSE "date_taken"
  ...
  if (fieldsToUpdate.order) {
    batchUpdateSQL = sql`${batchUpdateSQL}, "order" =
      CASE "id"
        ${sql.join(
          photoUpdates.map(
            (update) => sql`WHEN ${update.id} THEN ${update.order}`
          ),
          sql` `
        )}
        ELSE "order"
